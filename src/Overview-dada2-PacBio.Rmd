...
setwd("/Users/krkptrc2/Desktop/hpcbio-git/icann-Nov2020-16S")
```
...
# the gs"ub here might be a bit brittle...
dadas <- as.data.frame(
...
Now combine all metadata together for analysis and add back to the class. Remove kit and water control.
...
```{r }
table(tax_table(physeq.pacbio)[,'Class'])['Chloroplast']
...
sample_data(physeq.filtered)$Treatment <- factor(sample_data(physeq.filtered)$Treatment,levels = c("C","S","S_D","PS","PS_D"))
...
p
#ggsave("alpha-rarefaction-byTreatment.pdf", path = "./results/final-plots", device = pdf, width = 7.5, height = 5.5, units = "in")
```
...
```{r }
p <- p + facet_wrap(~Tissue) + ggtitle("Alpha Rarefaction by Tissue") + theme(legend.position = "right")
p
#ggsave("alpha-rarefaction-byTreatment-separatedbyTissue.pdf", path = "./results/final-plots", device = pdf, width = 6, height = 4, units = "in")
```
...
```{r }
p <- p + facet_wrap(~DFM) + ggtitle("Alpha Rarefaction by DFM")
p
```
...
Split phyloseq object into two: Mucosa and Digesta
```{r}
mucosa <- subset_samples(physeq.filtered, Tissue=="Mucosa")
digesta <- subset_samples(physeq.filtered, Tissue=="Digesta")
long.labels <- c("Control","Salmonella","Salmonella + DFM","PRSV/Salmonella","PRSV/Salmonella + DFM")
```

## Mucosa
```{r}
# all six measures: Observed, Chao1, ACE, Shannon, Simpson, Inv. Simpson, Fisher
erDF <- estimate_richness(mucosa)

# add Faith's PD, this is a simple wrapper to format everything correctly
pd <- estimate_pd(mucosa)
...
Pull out just Faith PD for Treatment
```{r}
p <- plot_richness_estimates(physeq = physeq.filtered, erDF = erDF[10], "Treatment", color = "Treatment", title="Alpha diversity in different treatment groups (Mucosa)")

p + geom_boxplot(aes(x=Treatment, y=value, color=NULL), alpha=0.1) +
  stat_compare_means(label = "p.signif", ref.group = "C",  hide.ns = TRUE) +
  ylab("Faith PD") + scale_x_discrete(labels=long.labels) + theme(legend.position = "none")
...
```{r}
# Remove controls
no.control <- subset_samples(physeq, DFM!="Control")
nc.erDF <- estimate_richness(no.control)

# add Faith's PD, this is a simple wrapper to format everything correctly
nc.pd <- estimate_pd(no.control)

# we don't need SR, it's the same as observed
nc.erDF$PD <- nc.pd$PD
nc.measures = colnames(nc.erDF)
```
```{r}
p <- plot_richness_estimates(physeq = no.control, erDF = nc.erDF[10], "DFM", color = "DFM", title="Alpha diversity in the presence and absence of DFM (Mucosa)")

p + geom_boxplot(aes(y=value, color=NULL), alpha=0.1) +
  stat_compare_means(label = "p.signif", ref.group = "Absent",  hide.ns = TRUE) +
  ylab("Faith PD") + theme(legend.position = "none")

#ggsave("alpha-faithpd-mucosa-byDFM.pdf", path = "./results/final-plots", device = pdf, width = 5.5, height = 4.5, units = "in")
```
...
```{r}
# all six measures: Observed, Chao1, ACE, Shannon, Simpson, Inv. Simpson, Fisher
erDF <- estimate_richness(digesta)

# add Faith's PD, this is a simple wrapper to format everything correctly
pd <- estimate_pd(digesta)
...
Pull out just Faith PD for Treatment
```{r}
p <- plot_richness_estimates(physeq = physeq, erDF = erDF[10], "Treatment", color = "Treatment", title="Alpha diversity in different treatment groups (Digesta)")

p + geom_boxplot(aes(x=Treatment, y=value, color=NULL), alpha=0.1) +
  stat_compare_means(label = "p.signif", ref.group = "C",  hide.ns = TRUE) +
  ylab("Faith PD") + scale_x_discrete(labels=long.labels) + theme(legend.position = "none")

#ggsave("alpha-faithpd-digesta-byTreatment.pdf", path = "./results/final-plots", device = pdf, width = 6, height = 5.5, units = "in")
```
...
```{r}
p <- plot_richness_estimates(physeq = no.control, erDF = nc.erDF[10], "DFM", color = "DFM", title="Alpha diversity in the presence and absence of DFM (Digesta)")

p + geom_boxplot(aes(y=value, color=NULL), alpha=0.1) +
  stat_compare_means(label = "p.signif", ref.group = "Absent",  hide.ns = TRUE) +
  ylab("Faith PD") + theme(legend.position = "none")

#ggsave("alpha-faithpd-digesta-byDFM.pdf", path = "./results/final-plots", device = pdf, width = 5.5, height = 4.5, units = "in")
```
...
```{r}
p <- plot_richness_estimates(physeq = physeq, erDF = erDF[10], "Tissue", color = "Tissue", title="Alpha diversity based on tissue type")

p + geom_boxplot(aes(x=Tissue, y=value, color=NULL), alpha=0.1) +
  stat_compare_means(label = "p.signif", ref.group = "Digesta",  hide.ns = TRUE) +
  ylab("Faith PD") + theme(legend.position = "none")

#ggsave("alpha-faithpd-all-byTissue.pdf", path = "./results/final-plots", device = pdf, width = 4.5, height = 4.5, units = "in")
```
...
```{r}
#Rearrange treatment order
sample_data(physeq.prev)$Treatment <- factor(sample_data(physeq.filtered)$Treatment,levels = c("C","S","S_D","PS","PS_D"))
sample_data(physeq.prev)$DFM <- factor(sample_data(physeq.filtered)$DFM,levels = c("Control","Absent","Present"))
```
...
#Pull out Mucosa & digesta only
mucosa.prev <- subset_samples(physeq.prev, Tissue=="Mucosa")
digesta.prev <- subset_samples(physeq.prev, Tissue=="Digesta")

mucosa.prop <- transform_sample_counts(mucosa.prev, function(x) x/sum(x) )
sample_data(mucosa.prop)$SampleSums <- sample_sums(mucosa.prev)
digesta.prop <- transform_sample_counts(digesta.prev, function(x) x/sum(x) )
sample_data(digesta.prop)$SampleSums <- sample_sums(digesta.prev)
```
...
#physeq.active <- mucosa.prop
physeq.active <- digesta.prop
```
...
p1 + ggtitle("Bray Curtis distance, Mucosa") 
...
sample_data(physeq.test)$DFM <- as.factor(c("Absent","Absent","Absent","Absent", "Absent","Absent", "Present", "Present", "Present", "Present", "Absent","Absent","Absent","Absent", "Present", "Present","Present", "Present", "Absent","Absent"))
#sample_data(physeq.test)$PRRSV <- as.factor(c("Control",  "Control", "Control",  "Control",  "Absent",  "Absent",  "Absent",  "Absent","Absent",  "Absent",  "Present", "Present", "Present", "Present", "Present", "Present","Present", "Present", "Control",  "Control","Control",  "Control",  "Absent",  "Absent","Absent",  "Absent", "Absent",  "Absent",  "Present", "Present","Present", "Present","Present", "Present", "Present", "Present","Absent",  "Absent",  "Absent",  "Absent"))
set.seed(1245)
...
```{r}
adonis2(finalPhyseq.prop.dist.bc ~ Tissue,
...
```{r}
adonis2(finalPhyseq.prop.dist.bc ~ DFM*PRRSV*Treatment*Tissue ,
       data = finalPhyseq.meta)
```
...
```{r}
beta <- betadisper(finalPhyseq.prop.dist.bc, finalPhyseq.meta$DFM)
permutest(beta)
```
...
```{r }
finalPhyseq.prop.ano <- anosim(finalPhyseq.prop.dist.bc, finalPhyseq.meta$DFM)
finalPhyseq.prop.ano
```
